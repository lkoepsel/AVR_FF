    ...
    while atts < 5:
        try:
            config.ser = serial.Serial(
                config.port, config.baudrate, timeout=1, xonxoff=config.xonxoff
            )
            atts = 5
            ...
            if e.errno == 16:
                ...
                rs = Serial_CmdD()
                if rs == 0:
                    print(f"Discon Serial from port after {atts} attempt(s)")
                    sleep(0.6)
...
def parse_arg(config):
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="print transfer statistics",
    )
    parser.add_argument(
        "-v",
        "--verbose",
...
def warm_ready(c):
    ...
    while init_response != ready:
        ...
        time.sleep(int(c.newlinedelay) * 0.001)
...
def main():
    ...
    s = int(n[0] / et.total_seconds())
    if config.stats:
        print(f"\n{n[1]} lines, {et.total_seconds():4.2f} secs, {s} bytes/sec")
        # print(f'{et.total_seconds():4.2f} secs')
        # print(f'{s} bytes/sec')
...
def xfr(parent_fname, parent_lineno, config):
    ...
    try:
        ...
        for line in clean_orig:
            ...
            if (
                response.endswith(badline)
                or response.endswith(defined)
                or response.endswith(compile)
            ) and not response.startswith(del_marker):
                compile_error = True
                responses.append(response)
                table.add_row(str(lineno), line[:-1], str(response), style="error")
            else:
                table.add_row(str(lineno), line[:-1], str(response))
            time.sleep(int(config.newlinedelay) * 0.001)
