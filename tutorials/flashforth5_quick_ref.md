...
| `d>q` | Extend double to quad-cell number. ( d -- q ) Requires qmath.h to be loaded. PIC18, PIC24-30-33. |
...
For PIC18, these words require `core.txt`, `math.txt` and `qmath.txt`.
...
| `ut*` | Unsigned 32x16 to 48-bit multiply. ( ud u -- ut ) For PIC18 and PIC24-30-33. |
...
| Word | Description |
|------|-------------|
| `mset` | Set bits in file register with mask c. ( c addr -- ) PIC24-30-33, the mask is 16 bits. |
...
$ff constant latb           \ PortB latch for the PIC18.
...
| `>xa` | Convert a Flash virtual address to a real executable address. PIC24-30-33, ATmega ( a-addr1 -- a-addr2 ) |
| `xa>` | Convert a real executable address to a Flash virtual address. PIC24-30-33, ATmega ( a-addr1 -- a-addr2 ) |
...
| `frot` | Rotate top three float items. ( flt1 flt2 flt3 -- flt2 flt3 flt1 ) |
| `fnip` | Remove second float item. ( flt1 flt2 -- flt2 ) |
...
| `f<=` | Leave true if flt1 less than or equal to flt2. ( flt1 flt2 -- f ) |
...
| `fasin` | Arcsine of flt, radians. ( flt -- asin(flt) ) |
...
| `[i` | Enter Forth interrupt context. ( -- ) PIC18, PIC24-30-33. |
| `i]` | Enter compilation state. ( -- ) PIC18, PIC24-30-33. |
...
| `int!` | Store interrupt vector to table. ( xt vector-no -- ) PIC18: vector-no is dummy vector number (0) for high priority interrupts. PIC30: Alternate interrupt vector table in Flash. PIC33: Alternate interrupt vector table in RAM. PIC24H/PIC24F: Alternate interrupt vector table in RAM. PIC24FK: Alternate interrupt vector table in Flash. PIC24E: Main interrupt vector table in RAM. ATmega: Interrupt vector table in RAM. |
| `int/` | Restore the original vector to the interrupt vector table in flash. PIC30, PIC24FK. ( vector-no -- ) |
| `ivt` | Activate the normal interrupt vector table. ( -- ) Not PIC24E, dsPIC33E. |
